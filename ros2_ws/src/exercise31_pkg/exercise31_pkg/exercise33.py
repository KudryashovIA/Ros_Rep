...
class SubscriberClass(Node):
    ...
    def __init__(self):
        ...
        self.odom_sub = self.create_subscription(Odometry, 'odom', self.odom_callback, 10, callback_group=self.odom_group)
        self.scan_sub = self.create_subscription(LaserScan, 'scan', self.scan_callback, QoSProfile(depth=10, reliability=ReliabilityPolicy.BEST_EFFORT), callback_group=self.scan_group)
        self.timer = self.create_timer(0.5, self.timer_callback, callback_group=self.timer_group)
